...
def introduce_typing_errors(word, current_wpm, error_slowdown=6):
    ...
    return error_word, max(0, current_wpm)
...
def hold_and_release_key(key):
    ...
    random_key_delay = max(0, random.uniform(0.03, 0.07) - key_down_duration)
...
def type_text_at_wpm(text, target_wpm=200, stop_key='tab'):
    ...
    for word in text.split():
        ...
        for char in error_word:
            keyboard.write(char)
            key_delay = max(0, random.uniform(0.04, 0.07))
            time.sleep(key_delay)

        hold_and_release_key('space')

        word_pause = max(0, random.uniform(0.02, 0.03))
